...
_listdisks="$(fdisk -l| grep "Disk /dev/"| cut -d " " -f 2 | sed 's/:/\ /g')"
j="1"
...
if [[ $opt = 1 ]]; then
	...
	if [[ ! -b $_blockdevice ]]; then
		printf  "Error: The block device $_blockdevice you chose does not exist!%s\n" 1>&2
		exit 1
	fi
...
#			sslwipe 
		...
		[nN][oO]|[nN])
			...
			;;
		*)
			echo -e "You did not enter a valid option\n"
			;;
	esac
